...
export default async function AdminCoursesPage() {
  const [{ data: depsData }, { data: coursesData }, { data: unitsData }, { data: materialsData }] = await Promise.all([
    ...
    supabaseAdmin.from('materials').select('id, title, type, unit_id, content_json, link_url, attachment_paths')
      .eq('status', 'approved').order('created_at'),
  ])

  // Build unitMaterials map: unitId → materials[]
  const unitMaterials: Record<string, { id: string; title: string; type: string; contentText: string; linkUrl: string; attachmentPaths: string[] }[]> = {}
