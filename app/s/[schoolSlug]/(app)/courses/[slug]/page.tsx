      ...
      {trending.length > 0 && (
        <MotionFadeUp delay={0.1}>
          <div className="mb-5">
            ...
            <MotionStagger className="flex flex-col gap-1.5" staggerChildren={0.04} delayChildren={0.18}>
              {trending.map((m, i) => (
                <MotionItem key={m.id}>
                  <Link
                    href={`/courses/${slug}/units/${m.unitId}`}
                    className="glass rounded-xl px-4 py-2.5 flex items-center gap-3 hover:bg-white/[0.06] hover:border-white/[0.12] hover:translate-y-[-1px] transition-all group"
                  >
      ...
      <MotionFadeUp delay={0.18}>
        <div>
          ...
          {units.length === 0 ? (
            ...
            <MotionStagger className="flex flex-col gap-2" staggerChildren={0.05} delayChildren={0.05}>
              {units.map(unit => (
                <MotionItem key={unit.id}>
                  <Link
                    href={`/courses/${slug}/units/${unit.id}`}
                    className="card-hover glass rounded-xl px-5 py-4 flex items-center justify-between transition-all hover:bg-white/[0.07] hover:border-white/[0.13] hover:translate-y-[-2px] group"
                  >
