    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 text-center relative">
        <div className="max-w-2xl w-full">
          ...
          {tenant.prizeEnabled && (
            <MotionFadeUp delay={0.3}>
              <div className="flex items-center justify-center gap-2 mb-6">
                <Link href={`/s/${schoolSlug}/leaderboard`} className="inline-flex items-center gap-2 glass border border-amber-500/25 px-4 py-2 rounded-full text-sm hover:border-amber-500/50 hover:bg-amber-500/5 transition-colors animate-float-bob">
          ...
          <MotionStagger className="flex items-center justify-center gap-6 mt-14 flex-wrap" delayChildren={0.6} staggerChildren={0.1}>
            {[
              { label: 'Courses', value: '127+' },
              { label: 'Departments', value: '8' },
              { label: 'Free forever', value: '✓' },
            ].map(stat => (
              <MotionItem key={stat.label} className="glass px-5 py-2.5 rounded-xl flex items-center gap-3 hover:bg-white/[0.05] transition-colors">
